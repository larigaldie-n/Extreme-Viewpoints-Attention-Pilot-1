...
{
  if (!dir.exists(file.path("intermediate_data", "ET_fixations_only_fixed"))) {dir.create(file.path("intermediate_data", "ET_fixations_only_fixed"), recursive = TRUE)}
  ...
  for (i in seq_len(length(d_eye_tracking)))
  {
    ...
    write.csv(d_eye_tracking[[i]], file = file.path("intermediate_data", "ET_fixations_only_fixed", ET_files_names[i]), row.names = FALSE)
